...
在 `/pages` 下，创建 `fetch.tsx` 文件。用于测试网络请求。
...
创建 `/api/login.ts`，在其中创建后端接口，使用 post 请求。
...
默认情况下，Next.js 会预渲染每个页面，即预先为每个页面，生成 HTML 文件，而不是由客户端 JavaScript 来完成。

预渲染，可以带来更好的性能和 SEO 效果。

当浏览器加载一个页面时，页面加载依赖 JS 代码，执行 JS 代码后，会激活页面，使页面具有交互性。(此过程称 Hydration)
...
4. 在项目 build 过程中，经过以上步骤，一个静态页面就打包生成。
...
在其中使用 `getStaticProps` 方法，发送网络请求。
...
> 这种模式，如果要更新静态页面，要重新打包项目。

情况三：

页面路由（路径 paths）取决于外部数据：需要动态添加路由；

使用 Next.js 提供的 `getStaticPaths` 和 `getStaticProps` 函数。
...
在浏览器中，访问 `http://localhost:3000/books-detail-ssg/4`

#### 1.应用场景

建议尽可能使用静态生成，只需构建一次，并可应用于 CDN。

可以为多种类型的页面，使用静态生成，包括：
...
- Next.js 会根据每个请求预呈现一个页面。缺点是稍微慢一点；因为渲染页面，无法被 CDN 缓存；
- 但预渲染页面将始终是最新的。
...
例如，假设你的某个页面，需要预渲染频繁更新的数据（从外部 API 获取）；

你就可以编写 `getServerSideProps` 获取该数据，并将其传递给 Page `props` ，如下所示：
...
`getServerSideProps` 的运行时机：

- 首先，它仅在服务器端运行，从不在浏览器上运行；
- 再页面中，使用 `getServerSideProps`，则：
  - 当直接通过 URL 请求此页面时，`getServerSideProps` 在请求时运行；
  - 当通过 `<Link>` 或 `router` 切换路由，来请求页面时，服务器端会运行 `getServerSideProps`；
  - 页面将使用返回的 `props` 进行预渲染；

### 3.增量静态再生 ISR

Next.js 允许在构建网站后，创建或更新静态页面。
