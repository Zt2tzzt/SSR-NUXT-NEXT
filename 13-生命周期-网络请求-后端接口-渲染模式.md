...
当浏览器加载一个页面时，再加载依赖的 JS 代码，执行 JS 代码后激活页面，使页面具有交互性。此过程称 Hydration。
...
例如，假设某个页面，需要在服务端预渲染，并且需要频繁更新的数据（从外部 API 获取）；
...
- 首先，它仅在服务器端运行；
- 在页面中，使用 `getServerSideProps`
  - 在浏览器中，直接通过 URL 请求此页面时，`getServerSideProps` 在服务端接收到请求后运行；
  - 当通过 `<Link>` 或 `router` 切换路由，来请求页面时，服务器端会运行 `getServerSideProps`；
- 页面将使用改函数返回的 `props` 进行预渲染；

### 3.增量静态再生 ISR

Next.js 允许在构建静态页面后，创建或更新静态页面。
